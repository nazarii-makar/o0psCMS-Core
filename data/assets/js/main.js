var App = (function () {
    ...
    $(window).ready(function () {
        $(".flash-message-display > .alert").fadeIn();
    });
    ...
    function leftSidebarInit() {
        ...
        $(document).on("touchstart mousedown", function (e) {
            if (!$.isXs()) {
                if (!$(e.target).closest(leftSidebar).length) {
                    if (lastTargetSubMenu != false) {
                        $(lastTargetSubMenu).parent("li").removeClass("open").find("ul.sub-menu").removeClass("visible");
                    }
                    lastTargetSubMenu = false;
                }
            }
            if (!$(e.target).closest(leftSidebar).length && body.hasClass(config.openLeftSidebarClass)) {
                if (!$(e.target).closest(leftSidebar).length && !$(e.target).closest(topHeader).length) {
                    if (lastTargetSubMenu != false) {
                        $(lastTargetSubMenu).parent("li").removeClass("open").find("ul.sub-menu").removeClass("visible");
                    }
                    lastTargetSubMenu = false;
                    $(".sidebar-elements li ul").removeAttr("style");
                }
                cSidebar();
            }
        });

        var lastTargetSubMenu = false;
        ...
        $(".sidebar-elements li a", leftSidebar).on("click", function (e) {
            ...
            if (!$.isXs()) {
                ...
                subMenu.parent("li").toggleClass("open");
            } else {
                ...
                if ($open) {
                    ...
                    $open.find("ul.sub-menu").toggleClass("visible");
                }
                ...
                if ($el.parent().hasClass('open')) {
                    $el.next().slideUp({
                        duration: $speed, complete: function () {
                            ...
                            $el.parent().find("ul.sub-menu").toggleClass("visible");
                        }
                    });
                } else {
                    $el.next().slideDown({
            ...
            if ((lastTargetSubMenu != false) && (lastTargetSubMenu != e.currentTarget)) {
                $(lastTargetSubMenu).parent("li").removeClass("open").find("ul.sub-menu").removeClass("visible");
            }
            lastTargetSubMenu = e.currentTarget;
